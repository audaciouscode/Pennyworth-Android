...
public class KeepAliveService extends JobIntentService {
    public static final String ACTION_KEEP_ALIVE = "com.audacious_software.pennyworth.KeepAliveService.ACTION_KEEP_ALIVE";
    public static final int JOB_ID = 12343;

    @Override
    protected void onHandleWork(@NonNull Intent intent) {
        if (KeepAliveService.ACTION_KEEP_ALIVE.equals(intent.getAction())) {
            AppLogger.getInstance(this).log("pennyworth_keep_alive_called");

            try {
                ScheduleManager.getInstance(this).updateSchedule(false, null, true);
